...
function wqxiangxi(index){
	...
	YMLib.UI.createWindow('wqxx','养护中心项目详情','yhzxxx.jsp','wqxx',940,450);
}
function closes(str){
...
function kaigong(index){
	if(confirm("确认开工吗？")){
		var data=$("#datagrid").datagrid('getRows')[index];
		obj1=data;
		YMLib.UI.createWindow('wqxx','养护中心开工','wqgzkg.jsp','wqxx',700,330);
	}
}
function ykaigong(index){
		var data=$("#datagrid").datagrid('getRows')[index];
		obj1=data;
		YMLib.UI.createWindow('wqxx','养护中心开工详情','wqgzkg1.jsp','wqxx',700,330);
}
	function wangong(index){
		var data=$("#datagrid").datagrid('getRows')[index];
		obj1=data;
			YMLib.UI.createWindow('wqxx','养护中心完工','wqgzwg.jsp','wqxx',500,300);
		}	
...
function ybsb(index){
	var data=$("#datagrid").datagrid('getRows')[index];
	obj1=data;
	YMLib.UI.createWindow('wqxx1','养护中心月报信息','yhzxyb.jsp','wqxx1',1059,467);
}
...
function AddInfo(){
	YMLib.UI.createWindow('wqxx','养护中心月报添加','yhdzxybtj.jsp','wqxx',800,340);
}
function Showybxx(index){
	var data=$("#ybgrid").datagrid('getRows')[index];
	parent.obj=data;
	parent.YMLib.UI.createWindow('wqxx','养护中心月报详情','yhzxybxx.jsp','wqxx',800,340);
}
function Edityb(index){
	var data=$("#ybgrid").datagrid('getRows')[index];
	obj=data;
	YMLib.UI.createWindow('wqxx','养护中心月报编辑','yhdzxybxg.jsp','wqxx',800,340);
}
...
function tjyhdzxyb(){
	...
	var data = "&gcglyhdzx.wc_stz="+$("#tj_wc_stz").val()+"&gcglyhdzx.wc_qttz="+$("#tj_wc_qttz").val()
	+"&gcglyhdzx.zjdw_stz="+$("#tj_zjdw_stz").val()+"&gcglyhdzx.zjdw_qttz="+$("#tj_zjdw_qttz").val()
	+"&gcglyhdzx.qksm="+$("#tj_qksm").val()
	+"&gcglyhdzx.sbsj="+sbsj+"&gcglyhdzx.sbyf="+$("#tj_sbyf").datebox('getValue')
	+"&gcglyhdzx.jhid="+parent.parent.obj1.XMBM+"&yhtype="+yhtype;
	
	$.ajax({
		type:'post',
		url:'../../../../gcgl/insertYhzxYb.do',
		data:data,
		dataType:'json',
		success:function(msg){
			if(Boolean(msg)){
...
function xgyhdzxyb(){
	...
	var data = "&gcglyhdzx.wc_stz="+$("#xg_wc_stz").val()+"&gcglyhdzx.wc_qttz="+$("#xg_wc_qttz").val()
	+"&gcglyhdzx.zjdw_stz="+$("#xg_zjdw_stz").val()+"&gcglyhdzx.zjdw_qttz="+$("#xg_zjdw_qttz").val()
	+"&gcglyhdzx.qksm="+$("#xg_qksm").val()
	+"&gcglyhdzx.jhid="+parent.obj.jhid+"&gcglyhdzx.id="+parent.obj.id+"&gcglyhdzx.sbyf="+$("#xg_sbyf").datebox('getValue');
//	alert(data);
	$.ajax({
		type:'post',
		url:'../../../../gcgl/updateYhzxYb.do',
		data:data,
		dataType:'json',
		success:function(msg){
			if(Boolean(msg)){
...
function tjwqgzkg(){
	...
	+"&gcglyhdzx.jhid="+parent.obj1.XMBM;
	$.ajax({
		type:'post',
		url:'../../../../gcgl/insertYhzxkg.do',
		data:data,
		dataType:'json',
		success:function(msg){
			if(Boolean(msg)){
...
//	var jgzt='0';
	...
	var ybzt=$("#ybzt").val();
	$('#datagrid').datagrid({    
	    url:'/jxzhpt/gcgl/selectYhzxjhList.do',
	    striped:true,
	    pagination:true,
	    rownumbers:true,
	    pageNumber:1,
...
//	    	jgzt: jgzt,
	    ...
	    columns:[[
              {field:'c',title:'操作',width:250,align:'center',formatter:function(value,row,index){
  	        	if(row.KGZT=='1'){
  	        		return '<a style="text-decoration:none;color:#3399CC;" href="#" onclick="dingwei('+index+')">定位</a>    '+'<a style="text-decoration:none;color:#3399CC; href="#" onclick="wqxiangxi('+index+')">详细</a>    '+'<a style="text-decoration:none;color:#3399CC; href="#" onclick="ybsb('+index+')">月报</a>   ';
  	        	}else
  	        	return '<a style="text-decoration:none;color:#3399CC;" href="#" onclick="dingwei('+index+')">定位</a>    '+'<a style="text-decoration:none;color:#3399CC; href="#" onclick="wqxiangxi('+index+')">详细</a>    '+'<a style="text-decoration:none;color:#3399CC; href="#" onclick="ybsb('+index+')">月报</a>   ';
  	        }},
			...
			{field:'YDBMC',title:'原道班名称',width:100,align:'center'},
			{field:'LXZH',title:'所在路线桩号',width:100,align:'center'},
			{field:'LXMC',title:'所在路线名称',width:100,align:'center'},
...
function showYBlist(){
	...
	$('#ybgrid').datagrid({    
	    url:'/jxzhpt/gcgl/selectYhzxYbByJhid.do',
	    striped:true,
	    pagination:true,
	    rownumbers:true,
	    pageNumber:1,
...
function sbsjyb(index){
	...
	if(confirm("确认上报吗？")){
		...
		cxsfcgsjd(data1.id,mydate.getFullYear(),mydate.getMonth()+1,mydate.getDate(),'gcgl_yhdzx',nf1,yf1);
		$.ajax({
			type:'post',
			url:'../../../../gcgl/sbYhzxYb.do',
			data:data,
			dataType:'json',
			success:function(msg){
				if(Boolean(msg)){
