...
<script type="text/javascript">
	function shty(){
		...
		$.ajax({
			 type : "POST",
			 url : "/jxzhpt/jhgl/shtyWqgzshByIdbeatch.do",
			 dataType : 'json',
			 data : 'planwqgzsj.id='+parent.ob.id+"&planwqgzsj.sbbm="+$.cookie("unit")+"&planwqgzsj.sbthcd="+(sbthcd+2),
	...
	function shbty(){
		...
		$.ajax({
			 type : "POST",
			 url : "/jxzhpt/jhgl/shbtyWqgzshByIdbeatch.do",
			 dataType : 'json',
			 data : 'planwqgzsj.id=' +parent.ob.id+"&planwqgzsj.sbbm="+$.cookie("unit")+"&planwqgzsj.sbthcd="+sbthcd+"&planwqgzsj.shyj1="+$("#shyj2").val(),
			 success : function(msg){
				 if(msg){
					 alert('审核成功！');
					 parent.$("#grid").datagrid('reload');
