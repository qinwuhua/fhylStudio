...
function loadTable(){
	...
	$.ajax({
		type:'post',
		url:'/jxzhpt/qqgl/selectYbmrsjd.do',
		dataType:'json',
		data:"wqbzbz.xmlx="+$("#xmlx").combobox('getValue'),
		success:function(data){
			var tbody="";
			for(var i=0;i<data.length;i++){
				array[i]=data[i];
				tbody=tbody+"<tr><td><a href='#' onclick=edit('"+i+"')>编辑</a>"
				+"<td>"+data[i].xmlx+"</td>"+"<td>"+data[i].yf+"</td>"
...
function edit(index){
	obj=array[index];
	YMLib.UI.createWindow('lxxx','请填截止时间','ybmrsjd_xg.jsp','lxxx',300,200);
}
function del(id){
	if(confirm('您确认删除吗？'))
	$.ajax({
...
<div style="width:100%;">
    <div  style="height:34px;" border="false">
	    <div id="righttop">
			<div id="p_top">当前位置>&nbsp;系统管理>&nbsp;月报上报默认时间点</div>
		</div>
    </div>
    <div style="width:80%;margin-top:10px;text-align:center;">
    		<select id='xmlx' class="easyui-combobox">
...
<!--     		<a id="tjwqbz" iconCls="icon-add" href="javascript:void(0)"  class="easyui-linkbutton" style="margin:2px 3px 0px 0px;" plain="true"  onclick="YMLib.UI.createWindow('lxxx','危桥补助标准添加','wqbzbz_add.jsp','lxxx',400,300);">添    加</a> -->
    ...
    <div style="height:450px;margin-top:10px;margin-left:10px;overflow: auto;" >
    	<table width="100%" class="bgg" border="0" cellpadding="0" cellspacing="1">
    		<tr style="font-weight:700;font-size:16px;"><td width="16%">操作</td><td width="14%">项目类型</td><td width="14%">月份</td><td width="14%">截止时间</td></tr>
