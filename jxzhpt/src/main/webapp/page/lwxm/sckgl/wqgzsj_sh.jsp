...
<script type="text/javascript">
	function shty(){
		$.ajax({
			 type : "POST",
			 url : "/jxzhpt/wqgzsj/shtyWqgzsckById.do",
			 dataType : 'json',
			 data : 'jckwqgzsj.id=' +parent.obj.sckid+"&jckwqgzsj.sbbm="+$.cookie("unit")+"&jckwqgzsj.sbthcd="+($.cookie("unit2").length-2),
	...
	function shbty(){
		...
		$.ajax({
			 type : "POST",
			 url : "/jxzhpt/wqgzsj/shbtyWqgzsckById.do",
			 dataType : 'json',
			 data : 'jckwqgzsj.id=' +parent.obj.sckid+"&jckwqgzsj.sbbm="+$.cookie("unit")+"&jckwqgzsj.sbthcd="+($.cookie("unit2").length+2)+"&jckwqgzsj.shyj1="+$("#shyj2").val(),
			 success : function(msg){
				 if(msg){
					 alert('审核成功！');
					 parent.$("#grid").datagrid('reload');
