...
<script type="text/javascript">
	function shty(){
		$.ajax({
			 type : "POST",
			 url : "/jxzhpt/aqsmfh/afSckZs.do",
			 dataType : 'json',
			 data : 'xmkaqsmfh.id=' +parent.obj.id+"&xmkaqsmfh.shzt=已审核"+"&xmkaqsmfh.sbthbmcd="+($.cookie("unit2").length)+"&xmkaqsmfh.cszt=已审核"
			 +"&xmkaqsmfh.shyj=同意",
	...
	function shbty(){
		...
		$.ajax({
			 ...
			 url : "/jxzhpt/aqsmfh/afSckZs.do",
			 dataType : 'json',
			 data : 'xmkaqsmfh.id=' +parent.obj.id+"&xmkaqsmfh.shzt=未审核"+"&xmkaqsmfh.sbthbmcd="+sbthcd+"&xmkaqsmfh.cszt=未审核"
			 +"&xmkaqsmfh.shyj="+$("#shyj2").val(),
			 success : function(msg){
				 if(msg){
					 alert('审核成功！');
					 parent.$("#grid").datagrid('reload');
