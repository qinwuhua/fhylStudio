...
<script type="text/javascript">
	function shty(){
		$.ajax({
			 type : "POST",
			 url : "/jxzhpt/aqsmfh/afXmkCs.do",
			 dataType : 'json',
			 data : 'xmkaqsmfh.id=' +parent.obj.sckid+"&xmkaqsmfh.sbthcd="+($.cookie("unit2").length-2)+"&xmkaqsmfh.csyj=同意"+"&xmkaqsmfh.sbzt=已上报"+"&xmkaqsmfh.cszt=已审核",
	...
	function shbty(){
		...
		$.ajax({
			 ...
			 url : "/jxzhpt/aqsmfh/afXmkCs.do",
			 dataType : 'json',
			 data : 'xmkaqsmfh.id=' +parent.obj.id+"&xmkaqsmfh.sbthcd="+($.cookie("unit2").length+2)+"&xmkaqsmfh.csyj="+$("#shyj2").val()+"&xmkaqsmfh.sbzt=未上报"+"&xmkaqsmfh.cszt=未审核",
			 success : function(msg){
				 if(msg){
					 alert('审核成功！');
					 parent.$("#grid").datagrid('reload');
