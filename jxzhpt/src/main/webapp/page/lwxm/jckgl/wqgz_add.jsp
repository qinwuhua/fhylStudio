		...
		mustMatch: true,
  		...
  		formatItem : function(row, i, max) {
			...
			function(e, item) {
				...
				$("#qlmc,#qlzxzh,#gydw,#xzqhdm,#xzqhmc,#lxmc,#lxbm,#kjzc,#qlqc,#qlkd,#dkzdkj,#pddj,#xjgjnd,#akjfl,#sbjgxs,#bhnr,#bz,#xlxbm,#xqlmc,#xqlbm,#xzxzh").attr("value",'');
				ptx=item.ptx;
				pty=item.pty;
				$("#jd").val(item.ptx);
				$("#wd").val(item.pty);
				...
				$("#xsq").val(item.xsq);
				$("#szxz").val(item.szxz);
				$("#xlxbm").val(item.xqlbm.split('L')[0]);
				$("#xqlbm").val(item.xqlbm);
