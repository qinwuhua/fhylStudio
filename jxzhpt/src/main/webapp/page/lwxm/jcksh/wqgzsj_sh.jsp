...
<script type="text/javascript">
	function shty(){
		$.ajax({
			 type : "POST",
			 url : "/jxzhpt/wqgzsj/sjshtyWqgzsjById.do",
			 dataType : 'json',
			 data : 'jckwqgzsj.id=' +parent.obj.id+"&jckwqgzsj.shbm="+$.cookie("unit")+"&jckwqgzsj.sbthcd="+($.cookie("unit2").length-2),
	...
	function shbty(){
		...
		$.ajax({
			 type : "POST",
			 url : "/jxzhpt/wqgzsj/sjshbtyWqgzsjById.do",
			 dataType : 'json',
			 data : 'jckwqgzsj.id=' +parent.obj.id+"&jckwqgzsj.shbm="+$.cookie("unit")+"&jckwqgzsj.sbthcd="+($.cookie("unit2").length+2)+"&jckwqgzsj.shyj2="+$("#shyj2").val(),
			 success : function(msg){
				 if(msg){
					 alert('审核成功！');
					 parent.jckglWqgz();
