...
<script type="text/javascript">
	function shty(){
		var ids=parent.ob.id.split(',');
		var sbthcd=$.cookie("unit2").length;
		for(var i=1;i<ids.length;i++)
			sbthcd+=','+$.cookie("unit2").length;
		$.ajax({
			 type : "POST",
			 url : "/jxzhpt/aqsmfh/afXmkZsbeatch.do",
			 dataType : 'json',
			 data : 'xmkaqsmfh.id=' +parent.ob.id+"&xmkaqsmfh.zszt=已审核"+"&xmkaqsmfh.sbthcd="+sbthcd+"&xmkaqsmfh.cszt=已审核"
			 +"&xmkaqsmfh.zsyj=同意",
	...
	function shbty(){
		...
		$.ajax({
			 ...
			 url : "/jxzhpt/aqsmfh/afXmkZsbeatch.do",
			 dataType : 'json',
			 data : 'xmkaqsmfh.id=' +parent.ob.id+"&xmkaqsmfh.zszt=未审核"+"&xmkaqsmfh.sbthcd="+sbthcd+"&xmkaqsmfh.cszt=未审核"
			 +"&xmkaqsmfh.zsyj="+$("#shyj2").val(),
			 success : function(msg){
				 if(msg){
					 alert('审核成功！');
					 parent.$("#grid").datagrid('reload');
