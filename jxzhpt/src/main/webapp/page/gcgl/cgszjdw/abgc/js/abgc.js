...
function zjdw(index){
	var data=$("#datagrid").datagrid('getRows')[index];
	obj1=data;
	YMLib.UI.createWindow('wqxx1','资金到位情况','abgczjdw.jsp','wqxx1',900,500);
	//window.open("abgczjdw.jsp");
}
function zjdw__ck(index){
	var data=$("#datagrid").datagrid('getRows')[index];
	obj1=data;
	YMLib.UI.createWindow('wqxx1','资金到位情况','abgczjdw__ck.jsp','wqxx1',900,500);
	//window.open("abgczjdw.jsp");
}
function closes(str){
...
function addCgs(){
	YMLib.UI.createWindow('wqxx','资金到位添加','abgczjdwtj.jsp','wqxx',700,250);
}
function editCgs(index){
	var data=$("#zjgrid").datagrid('getRows')[index];
	obj=data;
	YMLib.UI.createWindow('wqxx','资金到位编辑','abgczjdwxg.jsp','wqxx',700,250);
}
//添加车购税
function tjabgccgs(){
	var myDate = new Date();
...
//	    	'gcglabgc.xdsj':$("#xdnf").val(),
	    ...
	    columns:[[
	        {field:'c',title:'操作',width:150,align:'center',formatter:function(value,row,index){
	        	return '<a style="text-decoration:none;color:#3399CC;" href="#" onclick="dingwei('+index+')">定位</a>      '+'<a style="text-decoration:none;color:#3399CC;" href="#" onclick="afxiangxi('+index+')">详细</a>    '+'<a style="text-decoration:none;color:#3399CC;" href="#" onclick="zjdw('+index+')">资金拨付</a>   ';
	        }},
	  	   {field:'gydw',title:'管养单位',width:150,align:'center'},
	        {field:'xzqhmc',title:'行政区划',width:120,align:'center'},
	        {field:'lxbm',title:'路线编码',width:120,align:'center'},
...
function shezhi(){
	var data="gcglwqgz.jhid="+parent.obj1.jhid+"&gcglwqgz.nf="+new Date().getFullYear()+"&gcglwqgz.id="+parent.obj1.jhid+"&gcglwqgz.tablename=gcgl_abgc";
	$.ajax({
		type:'post',
