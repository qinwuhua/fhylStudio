...
function sbsjyb(index){
	...
	var data='';
	if(yhjb.length==11){
		data="gcglgcgzgj.id="+data1.id+"&gcglgcgzgj.sfsj=否"+"&gcglgcgzgj.sfth=否";
	}
	if(yhjb.length==9||yhjb.length==8||yhjb.length==2){
		data="gcglgcgzgj.id="+data1.id+"&gcglgcgzgj.sfsj=是"+"&gcglgcgzgj.sfth=否";
	}
	if(confirm("确认上报吗？")){
		$.ajax({
			type:'post',
...
function thsjyb(index){
	var data1=$("#ybgrid").datagrid('getRows')[index];
	var data="gcglgcgzgj.id="+data1.id+"&gcglgcgzgj.sfsj=否"+"&gcglgcgzgj.sfth=是";
	if(confirm("确认退回吗？")){
		$.ajax({
			type:'post',
			url:'../../../../gcgl/sbGcgzgjYb.do',
