...
function xggcgzgjyb(){
	...
	var zbfzj=parent.$("#zbfzj").html(); 
	var zbf=parseFloat(zbfzj);
	var zwc=parseFloat(zwczj)+parseFloat(btz)+parseFloat(qttz)-parseFloat(parent.obj.wc_btz)-parseFloat(parent.obj.wc_stz)-parseFloat(parent.obj.wc_qttz);
...
function showYBlist(){
	...
	$('#ybgrid').datagrid({    
	    url:'../../../../gcgl/selectxjgcYbByJhid.do',
	    striped:true,
	    pagination:true,
	    rownumbers:true,
	    pageNumber:1,
...
function sbsjyb(index){
	...
	if(confirm("确认上报吗？")){
		...
		cxsfcgsjd(data1.id,mydate.getFullYear(),mydate.getMonth()+1,mydate.getDate(),'gcgl_xj',nf1,yf1);
		$.ajax({
			type:'post',
			url:'../../../../gcgl/sbXjYb.do',
			data:data,
