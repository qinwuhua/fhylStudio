...
<script type="text/javascript">
	function addaqyb(){
		...
		var data= "gcgltz.tjdepartmentcode="+$("#tjdepartmentcode").val()+"&gcgltz.title="+$("#tj_title").val()+"&gcgltz.contens="+$("#tj_contens").val()+"&gcgltz.reportperuid="+$.cookie("unit")
		+"&gcgltz.id="+request('id')+"&gcgltz.reportper="+$.cookie("truename")+"&gcgltz.fsdwmc="+$("#fsdwmc").val();
		//alert(data);
		$.ajax({
				type:'post',
				url:'../../../../gcgl/insertTz.do',
				data:data,
				dataType:'json',
				async:false,
				success:function(msg){
					if(Boolean(msg)){
						flagadd=true;
						alert('请勿关闭窗口，等待文件上传成功后自动关闭，如未上传文件，请手动返回！');
						closeck();
					}else{
						alert('保存失败！');
					}
