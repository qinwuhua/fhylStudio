...
/* 			loadBmbm2('mqidj','MQI等级') */
		...
		function queryYhdzx(){
			...
			grid.columns=[[
				...
				{field:'cz',title:'操作',width:180,align:'center',
					formatter: function(value,row,index){
						...
						//if(row.sqzt==0 || row.sqzt==9 || row.sqzt==11){
							...
							"'/jxzhpt/page/qqgl/jhsh/yhdzx_edit.jsp'"+",980,400,'"+row.xmbm+"'"+')" style="color:#3399CC;">编辑</a>';
						//}else{
			    ...
			    {field:'pqidj',title:'PQI等级',align:'center',formatter:function(value,row,index){ 
			    	 var strs = value.split(",");    	 
			    	 var arr = new Array(); 
			    	 if(strs.length > 0){
			    	     for(i=0;i<strs.length;i++){
			    	    	 var result = '<a href="javascript:openPqiDetail('+"'"+index+"',"+"'"+strs[i]+"'"+')" style="color:#3399CC;">'+strs[i]+'</a>';
			    	    	 arr.push(result);
			    	    }
			    	     return arr.join(',');
			    	 }else{
  ...
  function deleteYhdzx(){
		...
		function openPqiDetail(index,flag){
			var data=$("#grid").datagrid('getRows')[index];
			YMLib.Var.Obj=data;
			YMLib.Var.sq=flag;
