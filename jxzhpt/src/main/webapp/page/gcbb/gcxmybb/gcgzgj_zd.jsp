...
<script type="text/javascript">
	...
	var xzqhdm=parent.$("#xzqh").combotree("getValues");
		...
		$("#save_button").click(function(){
			...
			if(kjgr){
				biaotstr+='<td rowspan="4">开工日--竣工日</td>';
				sql+="sjkgsj||'——'||sjwgsj v_"+k+',';
				k++;
			}
			if(ylmlx){
			...
			if(zygc){
				...
				sql+='dc v_'+k+',bywcdc v_'+(k+1)+',bnwcdc v_'+(k+2)+',zjwcdc v_'+(k+3)+',decode(dc,0,0,round((zjwcdc/dc)*100,2))||'%' v_'+(k+4)+',jc v_'+(k+5)+',bywcjc v_'+(k+6)+',bnwcjc v_'+(k+7)+',zjwcjc v_'+(k+8)+',decode(jc,0,0,round((zjwcjc/jc)*100,2))||'%' v_'+(k+9)+',mc v_'+(k+10)+',bywcmc v_'+(k+11)+',bnwcmc v_'+(k+12)+',zjwcmc v_'+(k+13)+',decode(mc,0,0,round((zjwcmc/mc)*100,2))||'%' v_'+(k+14)+',';
				k=k+15;
			}
			if(tze){
				biaotstr+='<td colspan="6">投 资 额 完 成 情 况</td>';
				sql+='pfztz v_'+k+',gys v_'+(k+1)+',bywcje v_'+(k+2)+',bnwcje v_'+(k+3)+',zjwcje v_'+(k+4)+',decode(dc,0,0,round((zjwcje/pfztz)*100,2))||'%' v_'+(k+5)+',';
				k=k+6;
			}
			if(xxjd){
			...
			<tr  style="height: 200px;">
				<td style="background-color: #ffffff;width:34%" align="center" valign="top">
					<table border="0" style="margin-top: 0">
						...
						<tr>
							<td width="25%" style="background-color: #ffffff;" align="left">
								<input  type="checkbox" value="qdzh||'-'||zdzh" name="checkbox" class="part1"/>起讫点桩号</td>
							<td width="25%" style="background-color: #ffffff;" align="left">
