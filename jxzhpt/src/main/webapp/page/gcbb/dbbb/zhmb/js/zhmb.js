...
function exportExcel_zhjz(){
	...
	var param='sbthcd='+sbthcd+'&gydw='+gydw+
 	'&xmnf='+$('#xmnf').combobox('getValues').join(",");
	window.location.href="/jxzhpt/dbbb/exportExcel_zhjz.do?"+param;
}

function showAllkg(){
	$('#datagrid').datagrid({    
		url:'/jxzhpt/dbbb/selectZhkg.do',
		queryParams : {
		 	'sbthcd':function(){
		 		if($.cookie("unit2")=='______36') return 7;
		 		else return $.cookie("unit2").length;
...
function exportExcel_zhkg(){
	...
	var param='sbthcd='+sbthcd+'&gydw='+gydw+
 	'&xmnf='+$('#xmnf').combobox('getValues').join(",");
	window.location.href="/jxzhpt/dbbb/exportExcel_zhkg.do?"+param;
}
function showAllkn(){
	$('#datagrid').datagrid({    
	    url:'js/aqgl.json',
