package com.hdsx.jxzhpt.wjxt.bean;

import java.io.Serializable;
import java.util.Date;

public class Lktjb implements Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private String id;
	private String tbdw;
	private String tbnf;
	private String dwfzr;
	private String fzr;
	private String tbr;
	private String tbrq;
	private String tjb_id;
	private String biaoti;
	private String xm;
	private String bh;
	private String hj;
	private String ydl;
	private String ldl;
	private String zdl;
	private String cdl;
	private String cadl;
	private String yllv;
	private String pdjg;
	private String tiaojian;
	private int page;
	private int rows;
	private byte [] wjfile;
	
	public byte[] getWjfile() {
		return wjfile;
	}
	public void setWjfile(byte[] wjfile) {
		this.wjfile = wjfile;
	}
	public String getId() {
		return id;
	}
	public void setId(String id) {
		this.id = id;
	}
	public String getTbdw() {
		return tbdw;
	}
	public void setTbdw(String tbdw) {
		this.tbdw = tbdw;
	}
	public String getTbnf() {
		return tbnf;
	}
	public void setTbnf(String tbnf) {
		this.tbnf = tbnf;
	}
	public String getDwfzr() {
		return dwfzr;
	}
	public void setDwfzr(String dwfzr) {
		this.dwfzr = dwfzr;
	}
	public String getFzr() {
		return fzr;
	}
	public void setFzr(String fzr) {
		this.fzr = fzr;
	}
	public String getTbr() {
		return tbr;
	}
	public void setTbr(String tbr) {
		this.tbr = tbr;
	}
	public String getTbrq() {
		return tbrq;
	}
	public void setTbrq(String tbrq) {
		this.tbrq = tbrq;
	}
	public String getTjb_id() {
		return tjb_id;
	}
	public void setTjb_id(String tjb_id) {
		this.tjb_id = tjb_id;
	}
	public String getBiaoti() {
		return biaoti;
	}
	public void setBiaoti(String biaoti) {
		this.biaoti = biaoti;
	}
	public String getXm() {
		return xm;
	}
	public void setXm(String xm) {
		this.xm = xm;
	}
	public String getBh() {
		return bh;
	}
	public void setBh(String bh) {
		this.bh = bh;
	}
	public String getHj() {
		return hj;
	}
	public void setHj(String hj) {
		this.hj = hj;
	}
	public String getYdl() {
		return ydl;
	}
	public void setYdl(String ydl) {
		this.ydl = ydl;
	}
	public String getLdl() {
		return ldl;
	}
	public void setLdl(String ldl) {
		this.ldl = ldl;
	}
	public String getZdl() {
		return zdl;
	}
	public void setZdl(String zdl) {
		this.zdl = zdl;
	}
	public String getCdl() {
		return cdl;
	}
	public void setCdl(String cdl) {
		this.cdl = cdl;
	}
	public String getCadl() {
		return cadl;
	}
	public void setCadl(String cadl) {
		this.cadl = cadl;
	}
	public String getYllv() {
		return yllv;
	}
	public void setYllv(String yllv) {
		this.yllv = yllv;
	}
	public String getPdjg() {
		return pdjg;
	}
	public void setPdjg(String pdjg) {
		this.pdjg = pdjg;
	}
	public String getTiaojian() {
		return tiaojian;
	}
	public void setTiaojian(String tiaojian) {
		this.tiaojian = tiaojian;
	}
	public int getPage() {
		return page;
	}
	public void setPage(int page) {
		this.page = page;
	}
	public int getRows() {
		return rows;
	}
	public void setRows(int rows) {
		this.rows = rows;
	}
	@Override
	public String toString() {
		return "Lktjb [id=" + id + ", tbdw=" + tbdw + ", tbnf=" + tbnf
				+ ", dwfzr=" + dwfzr + ", fzr=" + fzr + ", tbr=" + tbr
				+ ", tbrq=" + tbrq + ", tjb_id=" + tjb_id + ", biaoti="
				+ biaoti + ", xm=" + xm + ", bh=" + bh + ", hj=" + hj
				+ ", ydl=" + ydl + ", ldl=" + ldl + ", zdl=" + zdl + ", cdl="
				+ cdl + ", cadl=" + cadl + ", yllv=" + yllv + ", pdjg=" + pdjg
				+ ", tiaojian=" + tiaojian + ", page=" + page + ", rows="
				+ rows + "]";
	}
	
	
}
