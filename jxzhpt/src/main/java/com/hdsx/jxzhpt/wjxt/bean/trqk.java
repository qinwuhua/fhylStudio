package com.hdsx.jxzhpt.wjxt.bean;

import java.io.Serializable;
import java.util.Date;

public class trqk implements Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private String id;
	private String gydw;
	private String qxrs;
	private String trqxjf;
	private String lq;
	private String sn;
	private String ss;
	private String bzd;
	private String gyy;
	private String lqlbl;
	private String wjj;
	private String zzj;
	private String zxqc;
	private String cstb;
	private Date sbtbxj;
	private String tbdw;
	private String tjr;
	private String shr;
	private String tbsj;
	
	private int page;
	private int rows;
	public String getId() {
		return id;
	}
	public void setId(String id) {
		this.id = id;
	}
	public String getGydw() {
		return gydw;
	}
	public void setGydw(String gydw) {
		this.gydw = gydw;
	}
	public String getQxrs() {
		return qxrs;
	}
	public void setQxrs(String qxrs) {
		this.qxrs = qxrs;
	}
	public String getTrqxjf() {
		return trqxjf;
	}
	public void setTrqxjf(String trqxjf) {
		this.trqxjf = trqxjf;
	}
	public String getLq() {
		return lq;
	}
	public void setLq(String lq) {
		this.lq = lq;
	}
	public String getSn() {
		return sn;
	}
	public void setSn(String sn) {
		this.sn = sn;
	}
	public String getSs() {
		return ss;
	}
	public void setSs(String ss) {
		this.ss = ss;
	}
	public String getBzd() {
		return bzd;
	}
	public void setBzd(String bzd) {
		this.bzd = bzd;
	}
	public String getGyy() {
		return gyy;
	}
	public void setGyy(String gyy) {
		this.gyy = gyy;
	}
	public String getLqlbl() {
		return lqlbl;
	}
	public void setLqlbl(String lqlbl) {
		this.lqlbl = lqlbl;
	}
	public String getWjj() {
		return wjj;
	}
	public void setWjj(String wjj) {
		this.wjj = wjj;
	}
	public String getZzj() {
		return zzj;
	}
	public void setZzj(String zzj) {
		this.zzj = zzj;
	}
	public String getZxqc() {
		return zxqc;
	}
	public void setZxqc(String zxqc) {
		this.zxqc = zxqc;
	}
	public String getCstb() {
		return cstb;
	}
	public void setCstb(String cstb) {
		this.cstb = cstb;
	}
	public Date getSbtbxj() {
		return sbtbxj;
	}
	public void setSbtbxj(Date sbtbxj) {
		this.sbtbxj = sbtbxj;
	}
	public String getTbdw() {
		return tbdw;
	}
	public void setTbdw(String tbdw) {
		this.tbdw = tbdw;
	}
	public String getTjr() {
		return tjr;
	}
	public void setTjr(String tjr) {
		this.tjr = tjr;
	}
	public String getShr() {
		return shr;
	}
	public void setShr(String shr) {
		this.shr = shr;
	}
	public String getTbsj() {
		return tbsj;
	}
	public void setTbsj(String tbsj) {
		this.tbsj = tbsj;
	}
	public int getPage() {
		return page;
	}
	public void setPage(int page) {
		this.page = page;
	}
	public int getRows() {
		return rows;
	}
	public void setRows(int rows) {
		this.rows = rows;
	}
	
}
