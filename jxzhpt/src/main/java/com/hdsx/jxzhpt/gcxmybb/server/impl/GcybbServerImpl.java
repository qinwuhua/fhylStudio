...
public class GcybbServerImpl extends BaseOperate implements GcybbServer {
	...
	public List<Excel_list> getXgjwcmxb(Gcglabgc gcglabgc) {
		String sql="create or replace view xgjwcmxb as"
			    ...
			    +" substr(jh.sjkgsj,0,4) v_32,substr(jh.sjwgsj,0,4) v_33,(nvl(cgs.cgs, 0) + nvl(cgs.gz, 0) + nvl(cgs.sz, 0) + nvl(cgs.zq, 0) +nvl(cgs.dk, 0) + nvl(cgs.jl, 0) + nvl(cgs.qt, 0) + nvl(cgs.dfzc, 0) +nvl(cgs.yhdk, 0)) v_34,"
