...
+" select "
        ...
        +" jz3.ssdctc v_66,jz3.wkglc v_67,round(jz4.wcmc/lx.xmlc*100,0)||'%' v_68,decode(jh.bbzzj,'0','0%',null,'0%',round(jz4.zycgs/jh.bbzzj*100,0)||'%') v_69,round(jz4.ztz/jh.pfztz*100,0)||'%' v_70,l.bz v_71,lx.xzqhdm v_72,'新建' v_73,lx.gydw gydw,lx.xzqhdm xzqh,lx.lxbm lxmc,substr(jh.xmbm,0,4) xmnf,lx.lxbm lxbm,(case when kgzt='0' then '未开工' when jgzt='1' then '完工' else '已开工' end) jzzt"
        +" from jhsh_xj jh,"
        +" cbsj_xj cb,"
        +" kxxyj_xj kx,"
        +" lxsh_xj l,"
       ...
       +" (select g.jhid,(sum(g.zjdw_btz)+sum(g.dwyhdk)+sum(g.dwgz)+sum(g.dwsz)+sum(g.zjdw_qttz)) ztz,sum(g.zjdw_btz) zycgs,(sum(g.dwyhdk)+sum(g.dwgz)+sum(g.dwsz)) dfbz,sum(g.dwyhdk) yhdk,"
      +"  ' 0' sttx,sum(g.zjdw_qttz) qttz,sum(g.wc_btz+g.yhdk+g.gz+g.sz+g.wc_qttz) xmwc,sum(g.bywcmc) wcmc,sum(lqlmwcqk) lq,sum(snlmwcqk) sn,sum(yiji) yiji,sum(erji) erji,sum(sanji) sanji,sum(siji) siji,sum(g.dwgz) gz,sum(g.dwsz) sz"
      +"   from gcgl_xj g group by g.jhid) jz4,sgt_xj sgt"
